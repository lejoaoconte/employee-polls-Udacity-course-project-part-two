...
interface OptionOneTwoRangeProps {
  width: number;
}

export const OptionOneRange = styled.div<OptionOneTwoRangeProps>`
  width: ${({ width }) => `${width}%`};
  height: 30px;
  background: #285430;
`;

export const OptionTwoRange = styled.div<OptionOneTwoRangeProps>`
  width: ${({ width }) => `${width}%`};
  height: 30px;
  background: #a47e3b;
`;
